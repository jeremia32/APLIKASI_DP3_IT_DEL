  ...
  const handleBackupClick = async () => {
    try {
      const res = await axios.get("http://localhost:5000/api/hasil-penilaian/Backup/user", {
        responseType: "blob", // Kalau API mengembalikan file
      });

      // Trigger download jika itu file
      const url = window.URL.createObjectURL(new Blob([res.data]));
      const link = document.createElement("a");
      link.href = url;
      link.setAttribute("download", "backup_user.xlsx"); // Ubah jika file namanya berbeda
      ...
      <Content style={{ padding: "20px 50px" }}>
        ...
        <Breadcrumb style={{ marginBottom: "20px" }}>
          <Breadcrumb.Item onClick={() => navigate("/")}>
            <HomeOutlined />
            <span style={{ marginLeft: "8px" }}>Dashboard Admin</span>
          </Breadcrumb.Item>
        </Breadcrumb>

        {/* Section: User Counts */}
        <div style={{ marginBottom: "40px" }}>
          <Title level={3}>Jumlah Data User Aktif</Title>
          <Segmented
            options={[
