...
import { Spin, message, Radio, Typography, Button, Card, Table, Tooltip, Modal } from "antd";
...
const FormPenilaian = () => {
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      jawaban: Object.entries(jawaban).map(([pertanyaan, nilai]) => ({ pertanyaan, nilai })),
    };
  ...
  return (
    <div style={{ background: "", minHeight: "100vh", padding: 30 }}>
      <div style={{ maxWidth: 900, margin: "auto", padding: 60, borderRadius: 8, background: "#fff", boxShadow: "0 2px 8px rgba(0, 0, 0, 0.1)" }}>
        <Title level={3} style={{ textAlign: "center", marginTop: -20, fontSize: 24, fontWeight: "bold", paddingBottom: 20 }}>
          Lembar Penilaian
        </Title>
        <Text style={{ fontSize: 16, color: "#555", padding: 20, textAlign: "center", lineHeight: "1.8" }}>
...
<br />
        ...
        {categories.map((cat) => {
          // prepare dataSource for each kategori
          ...
          return (
            <Card key={cat._id} style={{ background: "#ffffff", borderRadius: 8, marginBottom: 20 }}>
                ...
                columns={[
                  ...
                  },
                  {
                    ...
                    render: (pertanyaanId, row) =>
                      row.isAspekRow ? null : (
                        <Radio.Group onChange={(e) => handleChange(pertanyaanId, e.target.value)} value={jawaban[pertanyaanId]} style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                          {[1, 2, 3, 4, 5, 6, 7].map((n) => (
