...
export const getEvaluationsForPenilai = async (req, res) => {
  try {
    ...
    const evaluations = await Nilai360.find({ penilai: user._id, active: true }).populate("penilai", "nip username email prestasi role").populate("user", "nip username email role prestasi");
